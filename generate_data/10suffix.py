## Script for generating training data with ten suffix variants ##
...
endings = ["a", "ta","e","te","tu","u","o","to","i","ti"]
...
def generate_srs(stem, ending, stem_class):
    ...
    if stem_class.startswith("s") and ending in ("ta","te","tu","to","ti"):
        stem_mod = stem[:-1] + "t"
    else:
        stem_mod = stem
    # canonical
...
filename = "10suffix.tsv"

with open(filename, "w", newline="", encoding="utf-8") as tsvfile:
    writer = csv.writer(tsvfile, delimiter="\t")
    writer.writerow( [f"s{i}" for i in range(len(stems_list))] + ['p0','p1','p2','p3','p4','p5','p6','p7','p8','p9']) #writing the first row
    ...
    for stem in stems_list:
        ...
        for ending in endings:
            ...
            # Determine winner
            if stem_class == "t-irregular" and ending in ("a","e", "u","o","i"):
                winner_idx = 1
            elif stem_class == "s-irregular" and ending in ("a","e", "u","o","i"):
                winner_idx = 2
            else:
                winner_idx = 0
            ...
            for idx, cand in enumerate(candidates):
                ...
                if ending in ("a", "e", "u","o","i") and idx == 0:
                    vc = 1
                if idx == 1:
                    ident = 1
                if idx == 2:
