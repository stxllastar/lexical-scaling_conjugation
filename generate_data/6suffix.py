## Script for generating training data with six suffix variants ##
...
def generate_srs(stem, ending, stem_class):
    # Korean coda rule: s -> t for s-class stems with consonant-initial suffix
    if stem_class.startswith("s") and ending in ("ta","tu","te"):
        stem_mod = stem[:-1] + "t"
    else:
        stem_mod = stem
    # canonical
...
filename = "6suffix.tsv"

with open(filename, "w", newline="", encoding="utf-8") as tsvfile:
    writer = csv.writer(tsvfile, delimiter="\t")
    writer.writerow( [f"s{i}" for i in range(len(stems_list))] + ['p0', 'p1','p2','p3','p4','p5']) #writing the first row
    ...
    for stem in stems_list:
        ...
        for ending in endings:
            ...
            # Determine winner
            if stem_class == "t-irregular" and ending in ("a", "e", "u"):
                winner_idx = 1
            elif stem_class == "s-irregular" and ending in ("a", "e", "u"):
                winner_idx = 2
            else:
                winner_idx = 0
